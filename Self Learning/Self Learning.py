...
def main():
        ...
        count=0
        for i in range(num_episodes):  #Agent vs agent
                state = env.reset()
                done = False
                ep_rewards = 0
                ...
                while done == False:
                        ...
                        if state[int((action)/3)][(action)%3]==0:  #to eliminate output of an action to a cell that is already occupied.
                                next_state,reward,done = env.step2(action+1)  #step2 is created in environment to output O and no get opponent move.
                                ...
                                while state[int((action)/3)][(action)%3]!=0:
                                        action = tabular_epsilon_greedy_policy(epsilon_greedy_policy.Q, eps, currentstateval)
                        if state[int((action)/3)][(action)%3]==0:  #to eliminate output of an action to a cell that is already occupied.
                                next_state,reward,done = env.step1(action+1)    #step1 is created in environment to output X and no get opponent move.
                                list_actions.append(action)
                                list_rewards.append(reward)
                                stateval=''.join(list(next_state.flatten().astype(str)))
                                list_states.append(stateval)
